...
describe('Shared Gemini quota gate — callGeminiGenerateContent error classification', () => {
  ...
  let restoreFetch;
  beforeAll(() => {
    scope = evalInScope(extractFunctions('callGeminiGenerateContent') + '\n' + extractConstants('GEMINI_CALL_TIMEOUT_MS'));
  });
  ...
  it('classifies an abort (timeout) distinctly from a network error', async () => {
    ...
    const quickScope = evalInScope(
      extractFunctions('callGeminiGenerateContent').replace('GEMINI_CALL_TIMEOUT_MS', '10')
    );
  ...
  it('a genuine success returns ok:true with the text -- no grounding metadata in the shape at all anymore, removed along with the automated investigation that used it', async () => {
    mockFetch(async () => ({
      ok: true, status: 200,
  ...
  it('regression: the request body sent to Google never includes a tools/grounding field, even if a caller tried to pass useGrounding', async () => {
    let capturedBody = null;
    mockFetch(async (url, opts) => {
      capturedBody = JSON.parse(opts.body);
      return { ok: true, status: 200, json: async () => ({ candidates: [{ content: { parts: [{ text: 'hello' }] } }] }) };
    });
    await scope.callGeminiGenerateContent({ GEMINI_API_KEY: 'k' }, { model: 'gemini-3.6-flash', prompt: 'p', useGrounding: true });
...
describe('runGeminiDailyAnalysis — shared quota gate + non-throwing contract', () => {
  ...
  beforeAll(() => {
    scope = evalInScope(extractFunctions(
      'runGeminiDailyAnalysis', 'callGeminiGenerateContent',
    ...
    ) + '\n\n' + extractConstants('GEMINI_CALL_TIMEOUT_MS', 'GEMINI_SHARED_QUOTA_CONFIG', 'ANALYSIS_SECTIONS'));
  });
  afterEach(() => { if (restoreFetch) { restoreFetch(); restoreFetch = null; } });
...
describe('runLinkGeminiAnalysis — shared quota gate + non-throwing contract', () => {
  ...
  beforeAll(() => {
    scope = evalInScope(extractFunctions(
      'runLinkGeminiAnalysis', 'callGeminiGenerateContent',
      'reserveGeminiQuotaSlot', 'buildQuotaBucketKeys', 'utcDayBucket', 'utcHourBucket',
      'recordGeminiProviderCall', 'normalizeBias'
    ) + '\n\n' + extractConstants('GEMINI_CALL_TIMEOUT_MS', 'GEMINI_SHARED_QUOTA_CONFIG'));
  });
  afterEach(() => { if (restoreFetch) { restoreFetch(); restoreFetch = null; } });
